...
//
// Improvements made:
// - Waits for DOMContentLoaded before querying elements (safer if script is included in <head>).
// - Validates email format before sending.
// - Uses AbortController to timeout the fetch (15s).
// - Handles non-JSON responses gracefully and surfaces server error messages.
// - Prevents double-submits by tracking disabled state and restores button text reliably.
// - Defensive checks and no use of eval/new Function (CSP-friendly).
      ...
      try {
        ...
        if (resp.ok) {
          if (data && data.success) {
            ...
            // leave button enabled so user can re-save if needed; do not auto-revert immediately
            setTimeout(() => setButtonLoading(false), 1000);
          } else {
            // OK but response didn't include success flag
